...
export default function Search() {
  ...
  const allItems = useMemo<SearchResult[]>(() => {
    const varietyResults: SearchResult[] = VARIETIES.map((v) => ({
      ...
      subtitle: `${v.code} \u00b7 ${v.primaryLocation.district}`,
  ...
  return (
    <>
      ...
      <button
        ...
        className="inline-flex items-center gap-2 px-3 py-1.5 border border-warm-white/25 text-warm-white/80 hover:text-quarry-gold hover:border-quarry-gold transition-colors duration-300 group"
      >
        ...
        <span className="hidden lg:inline font-mono text-[10px] text-warm-white/45 group-hover:text-quarry-gold/70 ml-1 no-justify">
          ⌘K
        </span>
      </button>
